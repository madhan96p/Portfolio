...
function renderDashboard(data) {
  ...
  if (data.pool.wallet.remaining < 0) {
    ...
    statusEl.className =
      "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tighter bg-red-500/20 text-red-400 border border-red-500/50";
    if (overlay) overlay.classList.remove("hidden");
  } else if (isOverDailyLimit) {
    statusEl.innerText = "BURN RATE HIGH";
    statusEl.className =
      "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tighter bg-amber-500/20 text-amber-400 border border-amber-500/50";
    if (overlay) overlay.classList.add("hidden");
  } else {
    statusEl.innerText = "SYSTEM STABLE";
    statusEl.className =
      "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tighter bg-emerald-500/20 text-emerald-400 border border-emerald-500/50";
    if (overlay) overlay.classList.add("hidden");
  }
  ...
  document.getElementById(
    "family-actual"
  ).innerText = `₹${data.family.actual.toLocaleString()}`;
  document.getElementById(
    "family-goal"
  ...
  document.getElementById("wealth-ratio").innerText =
    data.summary.wealthToDebtRatio;

  // Progress Bar (Ensuring it doesn't go negative or over 100 visually)
  const familyProgress = Math.min(
    Math.max(0, parseFloat(data.family.progress)),
    100
  );
  document.getElementById(
    "family-progress-bar"
