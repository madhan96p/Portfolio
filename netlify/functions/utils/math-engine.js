...
const calculateFinancials = (config, transactions, portfolio) => {
    ...
    const familyGoal = (actualSalary * 0.60) + debtRollover;
    ...
    const familyBorrowed = transactions
        .filter(t => t.Category === 'Family Support' && (t.Entity.includes('Mom') || t.Entity.includes('Dad')))
        .reduce((sum, t) => sum + parseFloat(t.Amount_CR || 0), 0);
    ...
    const poolTotal = (actualSalary * 0.40) + openingBalance;

    // Sub-Pools
    const walletGoal = poolTotal * 0.50;
    ...
    const lifestyleSpent = transactions
        .filter(t => t.Category === 'Personal Spending' || t.Category === 'Household Spending')
        .reduce((sum, t) => sum + parseFloat(t.Amount_DR || 0), 0);
    ...
    return {
        summary: {
            actualSalary,
            wealthToDebtRatio: debtRollover > 0 ? (totalPortfolioValue / debtRollover).toFixed(2) : '∞',
            netLiquidity: openingBalance + actualSalary - (lifestyleSpent + familySent)
        },
        family: {
            goal: familyGoal,
        ...
        pool: {
            total: poolTotal,
            wallet: { goal: walletGoal, spent: lifestyleSpent, remaining: walletGoal - lifestyleSpent },
